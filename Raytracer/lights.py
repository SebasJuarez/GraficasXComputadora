...
def reflect(normal, direction):
    reflectValue = [2 * numeritos.dot_product(normal, direction) * n - d for n, d in zip(normal, direction)]
    return numeritos.vecNorm(reflectValue)

def totalInternalReflection(incident, normal, n1, n2):
    c1 = numeritos.dot_product(normal, incident)
...
def refract(normal, incident, n1, n2):
    ...
    t = numeritos.twoVecSubstraction(sp1, sp3)
    return numeritos.vecNorm(t)
...
class DirectionalLight(Light):
    def __init__(self, direction=(0, 1, 0), intensity=1, color=(1, 1, 1)):
        ...
        self.direction = numeritos.vecNorm(direction)

    def getDiffuseColor(self, intercept):
        direction = [i * -1 for i in self.direction]

        intensity = numeritos.dot_product(intercept.normal, direction) * self.intensity
    ...
    def getSpecularColor(self, intercept, viewPosition):
        ...
        viewDirection = numeritos.twoVecSubstraction(viewPosition, intercept.point)
        viewDirection = numeritos.vecNorm(viewDirection)

        intensity = max(0, min(1, numeritos.dot_product(reflectDirection, viewDirection))) ** intercept.obj.material.spec
...
class PointLight(Light):
    ...
    def getDiffuseColor(self, intercept):
        direction = numeritos.twoVecSubstraction(self.position, intercept.point)
        radius = numeritos.vecNormSimple(direction)
